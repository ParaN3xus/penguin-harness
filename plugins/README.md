...
Versions are dates with a sequence number — `YYYY.MM.DD.N` — on the manifest and on every skill a plugin ships (the manifest's dated version is the plugin's own; the package's npm version follows the release). A plugin whose skills teach the model to install an external library into the user's project pins that library in `plugin.json` `libraries` — `{ "<npm name>": "X.Y.Z" }`, the exact version its content was written for — and every install command in its skills names that version: the loader refuses a range, and core's tests hold the prose and the pin together. Skills follow the "index first, body on demand" design: only their metadata is injected into an Agent's system prompt; the Agent reads the full `SKILL.md` via shell when it actually needs it. Hook scripts are plain Node (builtins only): the harness runs them as subprocesses at the loop's hook points with `{ hook, session_id, trace_path }` on stdin and reads their JSON answer from stdout.
