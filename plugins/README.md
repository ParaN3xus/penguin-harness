...
`agent-company` is the whole toolkit of company mode, entry point included. `company-setup` is that entry point: the skill that creates an organization with the user, asking one question at a time and ending at `penguin org create`, leaving hiring and tickets to the CEO. `company-employee` is the protocol every desk and ticket session follows, and `company-ceo`, `company-hr` and `company-finance` are the playbooks of those titles. `company-mirror` is the sixth: the protocol a company of digital twins runs on instead, where every employee mirrors a real colleague and the company relays between people rather than filing tickets. Because the plugin is `preinstall: false`, `company-setup` is no longer on every Agent by default — an Agent gets it by installing `agent-company` from the library (the Agents page), and the CEO and the employees carry it already.

`agent-company-proposals` is the skill half of the company-proposals plugin pair (the code half, `@prismshadow/penguin-plugin-company-proposals`, is a module plugin a Project installs; see [PRFC-0016](https://github.com/Prism-Shadow/penguin-harness-design/pull/210)): `proposal-author` writes and revises a proposal — a short, abstract, paragraph-commentable description of a change — and asks a colleague to build it at once; `proposal-implementer` builds it on a `proposal/<n>-<slug>` branch, opens the PR against `dev`, feeds back where the code disagrees with the text and merges into `main` on approval; `proposal-tester` runs `dev` in batches and files a fix ticket for a merged proposal or a runtime feedback for one still open. Installed on the employees that hold those roles, never by default.
