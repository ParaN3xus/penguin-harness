...
- candidate_round_limit：\`5\``,
  ...
  company: {
    ...
    proposals: {
      title: "提案",
      info: "一份提案是一件改动的说明：拟改的接口、改法、目的与一个测试例，由你委托的员工来写；另一名员工同时实施，PR 挂在关联材料里。逐段评论后点「请求整改」一次发给作者；认可即请求合并。链接写作 proposal:<编号>。",
      newProposal: "新建提案",
      newTitle: "新建提案",
      author: "作者",
      authorHint: "写这份提案的员工；它会在提案频道收到你的委托",
      brief: "委托",
      ...
      pickOne: "从左侧队列选一份提案",
      loadFailed: "提案加载失败",
      unreadBadge: (n: number): string => `${n} 条未读`,
      unreadNote: (n: number): string => `${n} 条未读事件`,
      status: {
        drafting: "起草中",
        ready: "已就绪",
        approved: "已认可",
      ...
      commentPlaceholder: "对这一段说点什么…",
      addComment: "添加评论",
      commentAdded: "评论已记下，点「请求整改」发给作者",
      pending: "待发",
      resolved: "已解决",
      resolvedNote: (text: string): string => `已解决：${text}`,
      fromRevision: (n: number): string => `第 ${n} 修订的段落`,
      showResolved: (n: number): string => `${n} 条已解决`,
      requestChanges: (n: number): string => (n > 0 ? `请求整改（${n}）` : "请求整改"),
      requestChangesTitle: "请求整改",
      requestChangesConfirm: (n: number): string =>
        `把 ${n} 条待发评论合成一批发给作者？作者会逐条处理并发布新的修订。`,
      changesRequested: "已发给作者",
      approve: "认可并请求合并",
      approveTitle: "认可提案",
      approveConfirm: (title: string): string =>
        `认可「${title}」并请求合并？实施者会把它合入主干。`,
      approved: "已认可，已通知实施者合并",
      reject: "拒绝",
      rejectTitle: "拒绝提案",
      rejectConfirm: (title: string): string => `拒绝「${title}」？`,
      rejectReason: "理由",
      rejectReasonHint: "一句话，记在事件里，作者能看到",
      rejected: "已拒绝",
      markMerged: "标记已合并",
      markMergedTitle: "标记已合并",
      markMergedConfirm: (title: string): string => `把「${title}」标记为已合并？`,
      merged: "已标记为已合并",
      ...
      event: {
        ...
        ready: "标记为已就绪",
        changes_requested: (n: number): string => `请求整改（${n} 条评论）`,
        implementation_started: (who: string): string => `请 ${who} 开始实施`,
        material_added: (label: string): string => `添加了材料 ${label}`,
        feedback: "发来实施反馈",
        runtime_feedback: "发来运行时反馈",
        resolved: "解决了一条评论",
        approved: "认可并请求合并",
        merged: "报告已合并",
        rejected: "拒绝了提案",
      ...
      capsuleUnread: (n: number): string => `${n} 条未读`,
      openProposal: "打开提案",
    },
  },
  errors: {
