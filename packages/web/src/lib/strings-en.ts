...
- candidate_round_limit: \`5\``,
  ...
  company: {
    ...
    /** The proposals page (the company-proposals plugin): the queue, one proposal's detail, its comments and the actions on it. */
    proposals: {
      title: "Proposals",
      info: "A proposal describes one change — the interfaces it touches, how, why, and one test case — written by the employee you delegate it to; a second employee builds it at the same time, its PR among the materials. Comment on paragraphs, then send them to the author in one batch with Request changes; approving requests the merge. Link one as proposal:<number>.",
      newProposal: "New proposal",
      newTitle: "New proposal",
      author: "Author",
      authorHint: "The employee that writes it; your brief reaches it in the proposals channel",
      brief: "Brief",
      ...
      dismissHint: "Got it",
      notFound: "Proposal does not exist",
      pickOne: "Pick a proposal from the queue",
      loadFailed: "Failed to load the proposal",
      unreadBadge: (n: number): string => `${n} unread`,
      unreadNote: (n: number): string => `${n} unread events`,
      status: {
        drafting: "Drafting",
        ready: "Ready",
        approved: "Approved",
      ...
      commentPlaceholder: "Say something about this paragraph…",
      addComment: "Add comment",
      commentAdded: "Comment noted — Request changes sends it to the author",
      pending: "Pending",
      resolved: "Resolved",
      resolvedNote: (text: string): string => `Resolved: ${text}`,
      fromRevision: (n: number): string => `A paragraph of revision ${n}`,
      showResolved: (n: number): string => `${n} resolved`,
      requestChanges: (n: number): string => (n > 0 ? `Request changes (${n})` : "Request changes"),
      requestChangesTitle: "Request changes",
      requestChangesConfirm: (n: number): string =>
        `Send ${n} pending comments to the author as one batch? The author works through them and publishes a new revision.`,
      changesRequested: "Sent to the author",
      approve: "Approve and request merge",
      approveTitle: "Approve the proposal",
      approveConfirm: (title: string): string =>
        `Approve "${title}" and request the merge? The implementer merges it into the main branch.`,
      approved: "Approved; the implementer is asked to merge",
      reject: "Reject",
      rejectTitle: "Reject the proposal",
      rejectConfirm: (title: string): string => `Reject "${title}"?`,
      rejectReason: "Reason",
      rejectReasonHint: "One line, recorded as an event the author sees",
      rejected: "Rejected",
      markMerged: "Mark merged",
      markMergedTitle: "Mark merged",
      markMergedConfirm: (title: string): string => `Mark "${title}" as merged?`,
      ...
      event: {
        ...
        changes_requested: (n: number): string => `requested changes (${n} comments)`,
        implementation_started: (who: string): string => `asked ${who} to implement it`,
        material_added: (label: string): string => `added material ${label}`,
        feedback: "sent implementation feedback",
        runtime_feedback: "sent runtime feedback",
