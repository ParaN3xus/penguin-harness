...
Agent 从不绑定模型：模型在创建 Session 时选定；同一个 Agent 可以在不同 Session 用不同模型运行。会话工具栏的模型选择器可在本会话内切换模型——先用当前模型压缩上下文，成功后本会话在新模型上继续，失败则保持原模型。会话内的 `/model` 命令则是**开新会话**换模型，按 handoff 方式进行：在同一 Agent 下新建一个使用新模型、沿用当前 Workspace 的会话，首条消息携带 `[model_switch_from]` 源块（源会话 id 与其 Trace 文件路径）——历史不注入新上下文（部分模型回放历史时必须携带 thinking 与 `fidelity`，不能跨模型），模型需要时按路径自行读取；原会话保持不变。`pricing` 三档价格供用量/成本中心按 Token 计费。
