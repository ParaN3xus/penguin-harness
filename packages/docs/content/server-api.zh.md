...
| Method | Path | 说明 |
| --- | --- | --- |
| POST | /api/browser/sites | 请求体 `{machineId, url}`——`url` 为键入的地址（`localhost:3000`、`example.com` 或完整 URL），`machineId` 为本对话 Workspace 所在机器（`null` = 本服务端）。返回 `{origin, url, address}`：站点所在的 `http://<label>.localhost:<port>` 主机、iframe 加载的 URL、规范化后的地址。`400` `invalid_url` / `unsupported_scheme` / `credentials_in_url`；机器端口另有 `403` `admin_required` 与 `404` `unknown_machine`；App 未经 `localhost` 访问时 `409` `browser_unavailable` |
...
**代理改写。** 请求：`Host` 为上游自己的名字，`Origin` / `Referer` 映射回上游 Origin，`Accept-Encoding: identity`。响应：删除 `X-Frame-Options`、CSP 的 `frame-ancestors` 与 HSTS，每条 `Set-Cookie` 去掉 `Domain=` 并改写为 `SameSite=None; Secure; Partitioned`（iframe 按设计与 App 跨站，浏览器在此只接受这种 Cookie，Lax 亦被拒绝），指向站点自身的 `Location` 改写到浏览器主机，HTML（≤ 8MB）在 `<head>` 最前注入 `<script src="/__penguin/browser.js">`（页面 CSP 带 nonce 时沿用）。
