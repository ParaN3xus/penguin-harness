...
openrouter、fireworks、siliconflow、tokendance、qwen-pay-as-you-go、qwen-token-plan 与 custom 分组走 OpenAI 兼容协议，因此复用 `OPENAI_*` 变量。opencode-go 分组中走 Chat Completions 与 Responses 的模型同样如此；该分组走 Anthropic Messages 的模型则读取 `ANTHROPIC_*`。直连 MiniMax M3 Responses 客户端使用 `MINIMAX_*`；内置 MiniMax 预设已固定官方端点。Provider 分组与内置模型目录见[模型与 Provider](/models)。
