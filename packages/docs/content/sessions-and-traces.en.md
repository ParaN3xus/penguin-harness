...
| Session | A continuous conversation under one (Agent, Workspace); the Workspace is locked at Session creation, and the model chosen then can be switched inside the Session (see "In-session model switch" below); ids look like `session-YYYY-MM-DD-HH-mm-ss-<8hex>` |
...
2. Read the runtime configuration from its `session_meta` — the Workspace, immutable for the lifetime of the Session, the model that context runs on, and the system prompt and thinking level that context opened with;
...
The model picker in an active conversation's toolbar switches the model within the same Session. A switch is a context rotation: the context is first compacted on the current model in summarize mode (regardless of the Agent's `compaction.mode: discard` — the summary has to reach the new model), then the next context opens on the target model in a new Trace file whose `session_meta` records that context's model. A compaction that fails or is aborted, or a summary that does not fit the target model's context window (ending `fatal`), keeps the current model. The compaction is an ordinary `manual` one — nothing on its `compaction_begin` / `compaction_end` pair names a model — and a context that was just compacted records no second pair. The new Trace file opens as soon as the switch completes, headed by its `session_meta` (then the summary, in summarize mode), and that record is also streamed; resume reads the model from this latest file, so a Session restarted right after a switch runs on the new model with the summary pending. The target must be in the Project configuration and constructible (credentials in place), or the switch is refused before any request; a Session that never ran has no context to compact and switches directly, writing nothing.

## New conversation on another model (/model)

The Web's `/model` command **opens a new session** that continues this conversation on another model, leaving this one as it is. It works the way the `/agent` handoff does: picking a model stages it in the composer, and sending creates a new Session under the same Agent via the ordinary session-creation API (the chosen model, **the source session's Workspace** — so files stay reachable), and the first message opens with a `[model_switch_from]` source block — the source session id, the absolute path of its latest Trace file, the Workspace, and the previous model pair — followed by whatever the user typed. The history is **not injected** into the new context: some models require thinking payloads and `fidelity` byte-for-byte when history is replayed, which cannot cross models — instead the model reads the source Trace file itself (JSONL, one message envelope per line) when it needs the earlier context. The source session and its Trace are untouched.
