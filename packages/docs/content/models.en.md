...
| --- | --- | --- |
| tokendance | `OPENAI_API_KEY` | The recommended vendor group. OpenAI-compatible gateway, preset base URL `https://tokendance.space/gateway/v1`; model ids are bare, with no vendor prefix (e.g. `glm-5.3`, `kimi-k3`); pricing is the gateway's own CNY rates, several of them currently discounted |
| penguin-go | `PENGUIN_GO_API_KEY` | Preset relay group with fixed base URL `https://token.penguin.ooo/api`; its header can authorize a key automatically or set one manually |
| opencode-go | `OPENAI_API_KEY` | OpenCode Go subscription gateway. Each model pins the protocol OpenCode's endpoint table gives it: Chat Completions or Responses at `https://opencode.ai/zen/go/v1`, Anthropic Messages at `https://opencode.ai/zen/go` (those rows read `ANTHROPIC_API_KEY`); see below |
...
The opencode-go group carries the 28 models OpenCode lists for its Go subscription. Their protocols differ, so each row pins its own `client_type` (`openai-chat`, `openai-responses` or `ant-messages`) and base URL, and a model added to the group by hand gets Chat Completions. One API key serves the whole group, so set it once with **Set key** on the group header. Go is billed monthly, and its usage limits are dollar amounts per model: a monthly allowance, capped at 20% per five hours and 50% per week. The rows record the per-token rates a request draws from that allowance, so for this group the cost center shows allowance used, not a per-token bill. Four rows store their base tier: `gpt-5.6-luna` above 272K input tokens, `grok-4.6` above 200K, and `qwen3.7-plus` and `qwen3.6-plus` above 256K. The four DeepSeek rows follow DeepSeek's off-peak schedule, described below. `union-alpha` is free for a limited time. Five models answer 403 until the key's OpenCode workspace opts in. Both Muse Spark Contributor models require consent to Meta training on prompts and completions, and are available only in the regions Meta permits. `deepseek-v4.1-flash`, `deepseek-v4-flash` and `deepseek-v4-pro` require consent to serving hosted in China.

Some models in the preset catalog: deepseek-flash / deepseek-v4-pro / deepseek-v4-flash / deepseek-v4-flash-vision-exp (`deepseek-flash` and `deepseek-v4-flash-vision-exp` read images; `deepseek-v4-flash` and `deepseek-v4-pro` are text-only. Since 2026-09-10 the two legacy Flash ids are retired names that DeepSeek serves from V4.1 Flash at the Flash price, which changes what they cost but not what V4 Flash reads: send an image to `deepseek-flash`), MiniMax-M3, gemini-3.8-flash, claude-opus-5 / claude-opus-4-8 / claude-sonnet-5, gpt-6-astra / gpt-5.6 / gpt-5.5, glm-5.3 / glm-5.3-flash, kimi-k3, qwen3.8-max / qwen3.8-flash, seed-2.1-pro / seed-2.1-turbo / seed-evolving, dots-3-note-preview (free on TokenDance, 512K context) (not exhaustive). Penguin Go's DeepSeek rows follow DeepSeek's current lineup and carry only `deepseek-flash` and `deepseek-v4-pro`. The whole OpenAI line-up is listed twice — directly (your own OpenAI key, list prices) and on OpenRouter as `openai/<id>` (the gateway's rates, which follow its running promotions). DeepSeek's direct-group rows record the official peak tier and declare its off-peak schedule: outside Beijing weekday 9:00–12:00 and 14:00–18:00 every bucket is halved, which the models page marks with a `50% off` badge and the cost center bills at. Three resold rows follow the same schedule, because their sellers pass DeepSeek's own windows through: TokenDance's `deepseek-v4.1-flash` and `deepseek-v4-flash-vision-exp`, and OpenRouter's `deepseek/deepseek-v4.1-flash`. The stored price is always the peak one, so what is on disk does not depend on the hour a Project was created or re-synced in. `glm-5.3-flash` appears four times, and all four rows accept images: AgentHub's GLM client forwards image parts for this one GLM id (every other GLM id refuses them), while the OpenRouter row `z-ai/glm-5.3-flash`, the TokenDance row and the OpenCode Go row go through the generic OpenAI-compatible clients, which carry them for any id. What the four rows do not share is the price: each records what its own seller charges, so they disagree while a promotion is running.

TokenDance rows record the gateway's list price and, where a promotion is running, the rate off it. Nine models are discounted today — `deepseek-v4-flash-0731`, `deepseek-v4-pro-0813` and `kimi-k3` at 20% off, `glm-5.3`, `glm-5.3-flash` and `qwen3.8-max` at 10%, and the three Doubao Seed rows (`seed-2.1-pro`, `seed-2.1-turbo`, `seed-evolving`) at 50%. Two further rows carry no flat discount and instead follow DeepSeek's own off-peak schedule, described above. Their model cards show the price being billed right now, with the rate as a badge. A Project is preset with the **list** price: the server stores the promotion beside it (in `web.db`, not in `.project_config.toml`) and takes it off when usage is priced, so the cost center charges what the gateway charges. Editing a row's price yourself cancels its promotion and keeps any off-peak decoration off the card: the figure is then yours, not the gateway's.
...
| `opencode.ai` | `x-opencode-session` | The Session's id |

Every other endpoint — every direct vendor, and every gateway that reads no such header — receives no extra headers at all. The headers carry nothing about the user or the Agent. OpenRouter's and TokenDance's state the app's identity only. OpenCode's names the conversation instead, because its gateway uses that value to route each conversation and serve its prompt cache: it holds still across a Session's requests, differs between Sessions, and is left off a request made outside any Session.

## Penguin Go preset group

`penguin-go` is a built-in model group, like TokenDance. New Projects contain the group's catalog models immediately; existing Projects add them through **Sync presets** on the Models page. The built-in rows provide a stable initial route to `https://token.penguin.ooo/api`, while the platform catalog returned during authorization or **Sync** can add model ids that are missing locally, together with their protocol, endpoint, display name, context window, vision capability, and platform price. Like every built-in group, synchronized models leave the user-owned `max_tokens` override unset and inherit the Agent setting. Existing rows refresh their three platform price buckets and client protocol; their endpoint and other Project-owned configuration are not overwritten, and rows are never deleted. Pure embedding rows are excluded. As in every other group, `.project_config.toml` holds the list price, and a flat promotion the platform runs is stored by the server in `web.db`, taken off when usage is priced, and shown as the model card's badge. These promotions come only from the platform: **Sync presets** does not set them, and each authorization or Sync replaces them. If that record is lost, usage is priced at the list price until the next authorization or Sync writes it back. The platform reports peak rates in USD per million tokens. The current built-in Penguin Go DeepSeek rows declare the same Beijing off-peak schedule as the direct DeepSeek group, so their model cards and cost records use half price outside weekday 9:00–12:00 and 14:00–18:00.

Before the group has a platform key, its **Authorize key** action creates a one-time request only after the user clicks it, then opens Penguin Go in a new tab while the local server polls. The browser receives only an authorization URL and local flow id; the device secret and delivered API key remain server-side. On success PenguinHarness validates `connection.apiKey`, the endpoints, and the model catalog. It writes the key to every existing group model, adds newly advertised models with their recommended protocol, endpoint, name, context window, vision capability, and list price, refreshes existing platform-model prices and client protocols, then replaces the group's promotions with the platform's. Synchronization preserves existing endpoints and other Project-owned configuration and never deletes models. Account and balance data are neither saved nor displayed.

After a platform key is configured, a separate **Sync** action appears immediately before **Add model**. **Authorize key** remains available so the owner can replace the key with one from another platform account. Sync uses the stored key to request the latest catalog, adds new models, refreshes existing platform-owned metadata, and leaves the config file untouched when the catalog produces no changes. If the platform reports that the key is invalid or revoked, the Models page automatically opens authorization again. Penguin Go's browser session and API key are separate: an expired browser session does not prevent a valid key from synchronizing; reauthorization is needed only when the key itself is invalid.

## Authorizing a new API key

A provider that supports automatic key authorization puts an extra action on its group header in the Models page: **Authorize key**. TokenDance and Penguin Go both do. A successful flow writes the new key to every model in that group, replacing whatever key those entries carry. After Penguin Go authorization succeeds, a separate **Sync** action incrementally discovers models the platform later makes available while authorization remains available for switching accounts.

Pressing it opens the provider's authorization page in a new tab, while the dialog in the Models page waits for and reports the result. TokenDance uses a PKCE callback and exchange; Penguin Go uses device authorization start and poll endpoints. That protocol difference stays server-side, so the user interaction and group-key result are the same.

The exchange runs entirely on the server: neither a PKCE verifier nor a device secret reaches the browser, and the minted key goes straight into the model table. An authorization delivers one key and expires at the deadline returned by the provider; PenguinHarness never keeps a local flow for longer than ten minutes.

TokenDance supports entering a one-time code when its redirect cannot return. Penguin Go is polled by the Models page and therefore has no manual-code mode.

Only the Project owner can start authorization. The full key never returns to the Web app. If writing a Penguin Go key locally fails, the server keeps that single delivery briefly so the write can be retried without authorizing again.
...
Detection tolerates the ways a base URL is usually mistyped: one `/v1` too many (`https://host/v1/v1`), one too few (`https://host` for an API served under `https://host/v1`), or a whole endpoint URL pasted from a provider's documentation (`/chat/completions`, `/responses`, `/messages`). It probes the normalized base first, then its neighbouring form — the trailing `/v1` removed when it has one, added when it has not — taking the three protocols in the usual order for each (six short probes at most), and the base URL field is rewritten to the form that actually answered, so what you save is the URL the protocol is really served at.
