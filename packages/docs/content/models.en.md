...
- **Opt-in.** Five models answer with an error until the key's OpenCode workspace opts in to them. `muse-spark-1.3-contributor` and `muse-spark-1.2-contributor` need consent to Meta training on prompts and completions, and are available only in the regions Meta permits. `deepseek-v4.1-flash`, `deepseek-v4-flash` and `deepseek-v4-pro` need consent to being served from China.
