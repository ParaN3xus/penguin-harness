...
应用打开即已登录。窗口自己完成登录，既没有要输入的密码，也没有可修改的密码，所以应用窗口里的**系统设置**没有带**修改密码**的**账户**页面。
...
一个数据目录同一时间只能由一个服务端使用。如果你的数据目录上已经有服务端在运行——`penguin web` 或 `penguin server` 实例，或是仍然开着的旧版应用——应用不会再起一个，而是附着到那个服务端，并自己完成窗口的登录，窗口照样不出现登录页。它能这样做，靠的是能读取数据目录；因此遇到读不了的数据目录时，窗口会停在登录页，并先弹窗说明数据目录和占用它的服务端。窗口能自己登录，却不能给这个只是附着上去的服务端设置密码：要拿到一个可以手动输入的密码（例如给访问同一个服务端的浏览器用），先停掉该服务端，再运行 `penguin server reset-admin-password`。
...
窗口万一真的出现了登录页，出路也是这条命令：桌面应用给自己生成的密码是随机的，从未显示过，页面上没有可输入的内容。
