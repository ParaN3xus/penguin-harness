...
On the **board**, clicking a card opens its ticket in place. Moving a ticket takes a long press: hold the card until it lifts, then drag it to another column — a quick drag does nothing, and on a touch screen the page scrolls as usual until the card lifts — or use the move control inside the ticket. Inside a ticket, the parent, a child ticket and a ticket session each open by clicking their title. A ticket session opens as its full conversation, and while it is on screen the sidebar keeps one row for it under **Temporary**, with a ✕ and a link back to the ticket; going anywhere else removes the row. Ticket sessions are never listed in the sidebar otherwise.
