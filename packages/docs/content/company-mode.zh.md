...
**看板**上点卡片即在原地打开工单，把卡片拖到另一列即移动工单。触屏上要先按住卡片直到它浮起再拖：手指直接划动会滚动页面。工单里的移动控件不用拖拽也能移动它。工单里的父工单、子工单与工单会话都点标题打开。工单会话以完整的会话页打开；它在屏幕上时，侧栏在「临时」下为它留一行，带 ✕ 与返回工单的链接，去往别处这一行即消失。除此之外，侧栏从不列出工单会话。
