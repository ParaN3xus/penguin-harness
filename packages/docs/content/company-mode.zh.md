...
**看板**上点卡片即在原地打开工单。移动工单要长按：按住卡片直到它浮起，再拖到另一列——不按住直接拖不会有任何动作，触屏上卡片浮起之前页面照常滚动——也可以在工单里用移动控件。工单里的父工单、子工单与工单会话都点标题打开。工单会话以完整的会话页打开；它在屏幕上时，侧栏在「临时」下为它留一行，带 ✕ 与返回工单的链接，去往别处这一行即消失。除此之外，侧栏从不列出工单会话。
