...
| | `agent-company-proposals` | Proposals for company mode (the `company-proposals` plugin's protocol) — three skills: `proposal-author` (write a short, abstract, paragraph-commentable proposal and revise it on feedback), `proposal-implementer` (build it on a `proposal/<n>-<slug>` branch, open the PR, feed back what the proposal did not foresee) and `proposal-tester` (test the dev branch in batches: a fix ticket for a merged proposal, a revision request for one not yet approved) (not preinstalled: install on the employees an organization delegates proposals to) |
