...
describe("httpPluginRegistry", () => {
  ...
  it("tries a failed connection again, and names the last cause after the final attempt", async () => {
    ...
    const flaky: typeof fetch = async () => {
      calls += 1;
      if (calls < 3) throw new TypeError("fetch failed", { cause: new Error("ECONNRESET") });
      return new Response(JSON.stringify([VALID_ENTRY]), { status: 200 });
    };
    const noWait = () => Promise.resolve();
    const entries = await httpPluginRegistry(url, { fetchImpl: flaky, delay: noWait }).index();
    expect(entries).toEqual([VALID_ENTRY]);
    expect(calls).toBe(3);
    ...
    await expect(
      httpPluginRegistry(url, { fetchImpl: dead, attempts: 2, delay: noWait }).index(),
  ...
  it("does not retry what the server answered", async () => {
    ...
    await expect(httpPluginRegistry(url, { fetchImpl: gone }).index()).rejects.toThrow(/HTTP 404/);
    expect(calls).toBe(1);
  });
