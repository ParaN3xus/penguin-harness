...
modifier checks, and the modifier follows the platform: ⌘ on macOS, Ctrl elsewhere. The terminal
tab closes on ⌘W on a Mac, where Ctrl+W now passes to the shell as readline's delete-word; Windows
and Linux keep Ctrl+W. The editors in the Files panel and the handbook save on ⌘S / Ctrl+S, no
longer on either. The terminal toggle stays Ctrl+` on every platform (⌘` is macOS's own window
cycling). Chords match on the physical key (`KeyboardEvent.code`), so a CJK IME, Shift, Caps Lock
and a macOS Option chord no longer change what a key means, and defaults move to the key that
types their letter on a non-US layout where the browser exposes one.
...
- Every place that shows a chord — the terminal tab's × tooltip, the panel picker's kbd, the save
  ...
  writes it (`⌘W`, `⌃\``; `Ctrl+W`), and updates when the binding changes.
