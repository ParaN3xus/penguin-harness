...
- **发行版只读，Windows 磁盘不可见。** 工作区按模式以读写或只读方式挂回其 `/mnt/<盘符>/…` 路径；一项设置可改为以只读方式显示所有磁盘。`network: "none"` 让命令处于空的网络命名空间。被屏蔽的目录挂 tmpfs，被屏蔽的文件挂 `/dev/null`。
...
- **安装在后端卡片上完成，并显示进度。** 安装 WSL 只弹出一次 Windows 授权，运行中实时显示 `wsl --install` 的输出。初始化下载基础 rootfs（校验 sha256）、导入、用 apt 或 apk 安装 bubblewrap 与软件包列表，无需管理员权限。检查隔离效果通过沙盒执行真实命令并逐项列出结果。移除发行版会将其注销。
- **命令在 Linux 中执行。** Windows 工具链无法在沙盒内运行；由发行版的软件包列表提供 Linux 版本。镜像设置可把 apt 或 apk 指向其他源。
