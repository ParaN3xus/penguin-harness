...
- **The distro is read-only and the Windows drives are hidden.** The Workspace is bound back in at its `/mnt/<drive>/…` path, read-write or read-only by mode. A setting shows every drive read-only instead. `network: "none"` leaves the command in an empty network namespace. A masked directory gets a tmpfs, a masked file `/dev/null`.
...
- **Setup happens on the backend's card, with progress.** Install WSL raises one Windows consent prompt and shows `wsl --install`'s output as it runs. Initialize downloads the base rootfs (checked against its sha256), imports it, installs bubblewrap and the package list through apt or apk, and needs no administrator. Check confinement runs real commands through the sandbox and lists each result. Remove distro unregisters it.
- **Commands run in Linux.** Windows toolchains cannot run confined; the distro's package list supplies Linux ones. A mirror setting points apt or apk elsewhere.
