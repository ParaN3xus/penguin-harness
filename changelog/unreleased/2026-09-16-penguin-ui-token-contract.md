...
value for value — the app renders pixel-identically. The one visible change is on first load: the
stored light/dark mode, accent and font size are applied before the first frame, so a dark page no
longer flashes white while the bundle loads and the first frame no longer renders at 16px and
reflows to the 18px default.
