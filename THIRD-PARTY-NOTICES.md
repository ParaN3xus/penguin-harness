...
## MiSans typeface — `packages/ui/src/fonts/misans/`

PenguinHarness uses [MiSans](https://hyperos.mi.com/font/), a typeface by Xiaomi Inc. (小米科技有限责任公司), unmodified:
Regular (400) and Medium (500) are the Latin and CJK typeface of the Frost theme and ship inside the
application as `unicode-range` woff2 slices (loaded only by a browser showing that theme), and MiSans Light
is used for the light (thin) weight in pages and screenshots. The license permits distributing an
application made with MiSans; the font files, or any component of them, are never distributed, sublicensed
or offered for download on their own. The application states that it uses MiSans (condition 1), and every
copy of the font files is accompanied by the copyright notice and this Agreement (condition 4): the text
below ships beside them as `packages/ui/src/fonts/LICENSES/misans.txt` and is emitted into every build.

Copyright (c) Xiaomi Inc. All rights reserved.

**License: MiSans Font Intellectual Property License Agreement (MiSans 字体知识产权许可协议),** as published
by the licensor in Chinese and English:
